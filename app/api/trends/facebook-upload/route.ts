...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Transform data to match FB data scraper table structure
    const rows = data.map((item: any) => ({
      ...
      "Shares Count": item["Shares Count"],
      "Post URL": item["Post URL"],
      "Account Name": item["Account Name"],
      "Account URL": item["Account URL"],
    ...
    if (error) {
      console.error('[Facebook Upload] Supabase error:', error.message)
